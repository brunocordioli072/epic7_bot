...
import epic7_bot.templates.hunt as hunt_templates
import time
import logging
...
def do_rotation():
    ...
    def check_buttons():
        try_again_button = screen.check_change_on_area(
            x1=1416, x2=1533, y1=810, y2=841, template=hunt_templates.try_again, percentage=0.6)
        confirm_button = screen.check_change_on_area(
            x1=1395, y1=808, x2=1492, y2=839, template=hunt_templates.confirm, percentage=0.6)
    ...
    if screen.check_change_on_area(
            x1=629, y1=175, x2=960, y2=224, template=hunt_templates.insufficient_energy, percentage=0.6) is not None:
