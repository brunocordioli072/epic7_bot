...
class Battle(Module):
    ...
    def has_energy(self):
        if (
            self.ScreenManager.match_template_on_screen_area(
                ...
                x2=960,
                y2=224,
                template=self.HuntTemplates.insufficient_energy,
                percentage=0.6,
            )
            is not None
        ):
    ...
    def do_hunt_rotation(self):
        ...
        if (
            self.ScreenManager.match_template_on_screen_area(
                ...
                x2=1492,
                y2=839,
                template=self.HuntTemplates.confirm,
                percentage=0.6,
            )
            is not None
        ):
    ...
    def start_hunt(self):
        if (
            self.ScreenManager.match_template_on_screen(
                template=self.HuntTemplates.pet_auto_battle_active, percentage=0.9
            )
            is None
        ):
            self.ScreenManager.random_click_on_area_and_check_change_retry(
                ...
                x2=749,
                y2=718,
                action="Click on Pet Auto Battle Active",
                percentage=20,
            )
