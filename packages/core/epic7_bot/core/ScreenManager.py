...
class ScreenManager(metaclass=Singleton):
    ...
    def get_position_of_template_match(self, matchTemplate):
        position_x = np.unravel_index(matchTemplate.argmax(), matchTemplate.shape)[1]
        position_y = np.unravel_index(matchTemplate.argmax(), matchTemplate.shape)[0]
    ...
    def match_template_on_screen_area_hsv(
        self, x1, y1, x2, y2, template, percentage=0.55
    ):
    ...
    def random_click_on_area(self, x1, y1, x2, y2):
        position_x, position_y = self.MathUtils.random_point_in_area(x1, y1, x2, y2)
        self.click_position(position_x, position_y, waitTime=0)
        # position_x, position_y = self.MathUtils.midpoint(x1, y1, x2, y2)
        # self.click_position(position_x, position_y, waitTime=0)

    def random_click_on_area_get_before_and_after_images_from_screen(
        self, x1, y1, x2, y2
    ):
    ...
    def random_click_on_area_and_check_change_on_screen_retry(
        self, x1, y1, x2, y2, action=None, percentage=70
    ):
        self.sleep(1)
        if action is not None:
        ...
        count = 0
        while (
            self.check_if_images_changed(
                beforeImage, afterImage, percentage=percentage, action=action
            )
            is False
            and count < 2
        ):
            (
                beforeImage,
                afterImage,
            ) = self.random_click_on_area_get_before_and_after_images_from_screen(
                x1, y1, x2, y2
            )
            count += 1
        if count < 2 == False:
    ...
    def random_click_on_area_get_before_and_after_images_from_area(
        self, x1, y1, x2, y2
    ):
    ...
    def random_click_on_area_and_check_change_on_area_retry(
        self, x1, y1, x2, y2, action=None, percentage=70
    ):
        self.sleep(1)
        if action is not None:
        ...
        count = 0
        while (
            self.check_if_images_changed(beforeImage, afterImage, percentage, action)
            is False
            and count < 2
        ):
            (
                beforeImage,
                afterImage,
            ) = self.random_click_on_area_get_before_and_after_images_from_area(
                x1, y1, x2, y2
            )
            count += 1
        return count < 2
    ...
    def ensure_not_on_sleep_mode_on_lobby(self):
        logging.info("Double Click on Screen to Ensure not on Sleep Mode on Lobby")
        self.random_click_on_area(x1=894, y1=848, x2=935, y2=879)
        self.sleep(1)
        self.random_click_on_area(x1=894, y1=848, x2=935, y2=879)
